...
class DesktopAppCreator {
    ...
    createDesktopApp(schoolName, schoolCode) {
        try {
            ...
            this.createElectronApp(appDir, schoolName, schoolCode);
    ...
    // Create Electron app structure
    createElectronApp(appDir, schoolName, schoolCode) {
        const electronApp = {
            name: `${schoolName} Library System`,
            version: "1.0.0",
        ...
        fs.writeFileSync(
            ...
            JSON.stringify(electronApp, null, 2)
        );
    }

    // Create main Electron process
    createMainProcess(appDir, schoolName, schoolCode) {
        const mainProcess = `const { app, BrowserWindow, Menu, shell, ipcMain } = require('electron');
const path = require('path');
...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  <text x="200" y="105" text-anchor="middle" fill="white" font-family="Arial" font-size="16" font-weight="bold">L</text>
        ...
        // Note: For production, you'd want to convert this to .ico, .icns, and .png formats
