...
class LibrarySystem {
    ...
    async fetchBooks() {
        const res = await fetch('/api/member-books');
    ...
    async renderMembers() {
        ...
        this.members.forEach((member, idx) => {
            ...
            admIdTd.classList.add('id-highlight');
    ...
    setupEventListeners() {
        document.getElementById('book-form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                this.showNotification('An error occurred while saving the book.', 'error');
            }
        });
        ...
        document.getElementById('member-form').addEventListener('submit', async (e) => {
            ...
            const admId = document.getElementById('member-admid').value.trim();
            if (!name || !admId) return;
            ...
            try {
                ...
                this.showNotification('An error occurred while saving the member.', 'error');
            }
        });
    ...
    async deleteBook(id) {
        const response = await fetch(`/api/member-books/${id}`, { method: 'DELETE' });
        if (response.ok) {
            await this.render();
        } else {
            const error = await response.json();
    ...
    async returnBook(bookId) {
        await fetch('/api/return', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ bookId, userType: 'member' })
        });
        await this.render();
    }
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            const matches = this.members
                .map((m, idx) => ({
                    ...
                    name: m.name,
                    admId: m.admId,
                    display: `${idx + 1}. ${m.name} (${m.admId || ''})`
                }))
                .filter(m => m.name.toLowerCase().includes(value) || (m.admId && m.admId.toLowerCase().includes(value)));
    ...
    showNotification(message, type = '') {
        ...
        // Show longer for detailed messages
        const duration = (typeof message === 'object' && message.details) ? 5000 : 2000;
        setTimeout(() => {
